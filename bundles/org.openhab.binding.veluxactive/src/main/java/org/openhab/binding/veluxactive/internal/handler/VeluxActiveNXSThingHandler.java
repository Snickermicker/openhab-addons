...
import org.openhab.binding.veluxactive.internal.config.VeluxActiveSensorConfiguration;
import org.openhab.binding.veluxactive.internal.dto.thermostat.RemoteSensorCapabilityDTO;
...
/**
 * The {@link VeluxActiveNXSThingHandler} is responsible for updating the channels associated
 * with an VeluxActive remote sensor.
 *
 * @author Mark Hilbush - Initial contribution
 */
@NonNullByDefault
public class VeluxActiveNXSThingHandler extends BaseThingHandler {
    ...
    private final Logger logger = LoggerFactory.getLogger(VeluxActiveNXSThingHandler.class);

    private @NonNullByDefault({}) String sensorId;

    private Map<String, State> stateCache = new ConcurrentHashMap<>();

    public VeluxActiveNXSThingHandler(Thing thing) {
        super(thing);
    }

    @Override
    public void initialize() {
        sensorId = getConfigAs(VeluxActiveSensorConfiguration.class).sensorId;
    ...
    public void updateChannels(RemoteSensorDTO sensor) {
        ...
        updateChannel(CH_SENSOR_ID, VeluxActiveUtils.undefOrString(sensor.id));
        updateChannel(CH_SENSOR_NAME, VeluxActiveUtils.undefOrString(sensor.name));
        updateChannel(CH_SENSOR_TYPE, VeluxActiveUtils.undefOrString(sensor.type));
        updateChannel(CH_SENSOR_CODE, VeluxActiveUtils.undefOrString(sensor.code));
        updateChannel(CH_SENSOR_IN_USE, VeluxActiveUtils.undefOrOnOff(sensor.inUse));
    ...
    private void updateCapabilityState(String capabilityType, String value) {
        ...
        switch (capabilityType) {
            case CAPABILITY_TEMPERATURE:
                try {
                    state = VeluxActiveUtils.undefOrTemperature(Integer.parseInt(value));
            ...
            case CAPABILITY_HUMIDITY:
                try {
                    state = VeluxActiveUtils.undefOrQuantity(Integer.parseInt(value), Units.PERCENT);
                } catch (NumberFormatException e) {
                    state = UnDefType.UNDEF;
                }
                break;
            case CAPABILITY_OCCUPANCY:
                state = VeluxActiveUtils.undefOrOnOff("true".equals(value));
                break;
            case CAPABILITY_ADC:
            case CAPABILITY_AIR_PRESSURE:
            case CAPABILITY_AIR_QUALITY:
            ...
            case CAPABILITY_VOC_PPM:
            default:
                state = VeluxActiveUtils.undefOrString(value);
                break;
        }
